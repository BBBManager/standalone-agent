...
public class EventRepository implements Serializable {
    ...
    private ConcurrentHashMap<String, ConcurrentLinkedQueue<Event>> eventQueuePerURL = new ConcurrentHashMap<>();
    ...
    public EventRepository() {
        ...
        Thread thPublishEvents = new Thread(new Runnable() {
            public void run() {
                log.info("Starting publish events thread");
                ...
                while (true) {
                    //log.debug("eventQueuePerURL contains " + eventQueuePerURL.size() + " urls");
                    for (String callbackUrl : eventQueuePerURL.keySet()) {
                        List<Event> events = EventRepository.getInstance().getAndRemoveAll(callbackUrl);

                        if (events.size() == 0) {
                            //log.debug("No events for URL " + callbackUrl);
                            events = null;
                            continue;
                        } else {
                            log.debug(events.size() + " events for URL " + callbackUrl);
                        }
